...
class ChatListDatabaseDataManager: ChatListDataManager {
    ...
    init() {
        ...
        guard let mainDictionaryFilePath = bundle.path(forResource: "TestData", ofType: "plist") else {
            return
        }
        guard let mainDictionaryFromFile = NSDictionary(contentsOfFile: mainDictionaryFilePath) else {
            return
        }
